  ...
  <div class="infinite-list-container" @scroll="scrollEvent" ref="_$refs">
    ...
    <div class="infinite-list" :style="{ transform: getTransform() }">
      <div
        ...
        :key="item[substitute.key]"
        :style="{
          height: _itemSize + 'px',
          lineHeight: _itemSize + 'px',
        }"
      >
        <!-- 这里可以完全自定义内部 -->
...
import { ref, defineProps, computed, onMounted, ComputedRef } from "vue";
type VisibleData = {
  [propName: number | string | symbol]: any;
};
...
let _visibleCount = 0;
// 偏移量
let _startOffset = ref(0);
// 起始索引
let _start = ref(props.start);
// 结束索引
let _end = ref(props.end);
// 列表item的高度
let _itemSize = ref(props.itemSize);
// 滚动盒子实例
let _$refs = ref();
...
}) as ComputedRef<VisibleData[]>;

function scrollEvent() {
  // 当前滚动位置
  const scrollTop = _$refs.value.scrollTop;
  // 此时的开始索引
  _start.value = Math.floor(scrollTop / _itemSize.value);
  // 此时的结束索引
...
onMounted(() => {
  _screenHeight.value =
    _$refs.value.parentNode.clientHeight ||
